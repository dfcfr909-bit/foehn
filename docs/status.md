...
- **気象庁の週間予報の信頼度を出すようにした（PR #53 / #54）。**
  「だいぶ先の予報は精度が怪しいと分かるように」という要望に、まず**リードタイムの目安**
  （PR #45）で応え、**峰では本物の信頼度に置き換えた**。
  `areas.json` の110峰に `office`（府県予報区）を持たせてある → `docs/spec/judge.md`
  - ⚠⚠ **アプリのA/B/Cは「登山適性」、気象庁のA/B/Cは「予報の信頼度」。**
    同じ字で出すと判定Cと読まれるので、画面は**高/中/低**、記号は `title` に回した
  - ⚠⚠ **「降水確率と気温の信頼度で、風の信頼度ではない」を必ず添える。**
...
案1（凡例に値）は `v4.81.0` で公開済み。**実機で凡例の効き具合を見てから**、
