...
- **タグを Actions から打てるようにした（PR #33）。** Claude からは `git push` でタグを
  送れない（403）ため、ここだけ人の手に残って v4.77.0 から止まっていた。
  版数は**手で打った値と HTML の表記の二重入力**で照合する → `docs/workflow.md`「タグ」
- **公開URLに配信されているものを機械で検証するようにした（PR #32）。**
  デプロイ後に sha256 を突き合わせる。⚠ **外部を叩く検査は手動のみ**にしてある
  （毎デプロイだと「赤いが直すところが無い」通知が出るため）
- **プロジェクトの取り違えを機械で防ぐようにした（PR #31）** → `CLAUDE.md` 冒頭
...
**#13（座標検証）は完了した。** 次の候補は2つ。どちらを先にするかは人に聞く。
...
>   直すのは**データ側**（分け方そのもの）
> - 受け入れ条件: `node tests/run-all.js` 全19件PASS

もう一つは **Issue #26（気圧配置）**。`loadWindGrid()` に `pressure_msl` を足し、
局所極値に H(青)/L(赤) を置いて等圧線を引く。調査済みで、できる／できないは
Issue に書いてある（**台風の複数AIモデル進路はデータ源が無く不可**）。
