...
- **判定に使う風の層を、モデル格子の標高で決めるのをやめた（PR #56 / ADR-0011）。**
  利用者から「至仏山で表示より現地の風が明らかに強かった」（2026-09-06）。
  引き直すと**予報は外れておらず、アプリが強い層を読んでいなかった**。
  判定に使った風 4.55 m/s で終日A、同じ日の800hPa は 22.78 m/s。
  - ⚠⚠ **110峰中109峰**が同じ状態だった。**ランキングでC・詳細画面でA**という
    食い違いも起きていた（詳細画面だけがモデル標高を渡していたため）
  - ⚠ **`judgePoint` / `THRESH` は一切変更していない。** 変えたのは入力の選び方だけ
  - ⚠ **既存の検査は安全弁を消しても全部通った**（別の理由で落ちていた）。
    `kind` だけでなく `reason` まで見るように直した → `docs/decisions.md`
  - 調べもの2本を Actions「外部の情報源を調べる」に足した。
    `wind-vs-report`（峰と日付を指定して過ぎた日を引き直す）/ `wind-elev-gap`（110峰を数える）
...
⚠⚠ **いちばん先にやること: 長押しでグラフが白く消える件（未解決）。**
`v4.81.0` の実機で、**長押しの最中に上3つのパネルが白くなり、窓も途中から描かれた**
（利用者のスクリーンショットあり）。⚠ **仕様ではない。**
`v4.82.0` で iOS の選択（虫眼鏡）を止めたので**それで消えるかをまず確かめる**。
残るなら、原因は選択UIではないので別に追うこと。

⚠ **いちばん先にやること: 案2（ポップアップをタップ時だけ出す）。**
案1（凡例に値）と窓の削減は `v4.82.0` で公開済み。**実機で効き具合を見てから**、
窓に何を残すか決める（体感温度・風向・気圧の変化量・確からしさ・ABC）。
判断材料は「凡例だけでどれくらい足りたか」。

> docs/status.md を読んだうえで、**案2（ポップアップをタップ時だけ出す）**を進めて。
> `origin/main` から新しいブランチを切ること。

⚠ **実機で至仏山も見てもらうこと（#16）。** ADR-0011 で判定に使う風が変わった。
ポップアップの風速の右が `@2228m` になっていれば効いている
