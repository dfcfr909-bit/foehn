...
| [#13](https://github.com/dfcfr909-bit/foehn/issues/13) | `areas.json` の座標を検証（**完了・PR #25**） | タスク |
...
- **`areas.json` の座標を全110峰検証し、95峰を山頂へ寄せた（#13・PR #25）。**
  着手時は28峰が許容差200m超。いまは**全110峰が許容差60m以内**（笙ヶ岳を除き機械で確定）
- **`snapPeaks.mjs` の吸着規則の誤りを直した（#13）。** 「elev に一致する最も近い点」は
  山頂でなく `elev−15m` の等高線を掴む。局所最高点だけを採るようにした
- **山名の表記揺れに対応した（#13）。** 「笙ヶ岳」で打つと岐阜の同名峰しか出ず
  500km先へ飛んでいた（地理院は「笙ガ岳」、OSMは「笙ケ岳」で登録）。
  揺れを作って引き、結果を**近い順**に並べる。`scripts/searchPeaks.mjs` も同様
