# 現状（最終更新: 2026-08-20 / タグは v4.77.0 のまま＝4機能ぶん遅れている）
...
- **検索結果に百名山の印と「高い順」トグルを足し、漢字違いの重複を畳んだ（PR #28）。**
  「五竜岳(OSM)」と「五龍岳(地理院)」が2行出ていたのを1行に
- **検索結果に標高を併記した（PR #27）。** 同名峰の選び分けは標高がいちばん確実
  （五竜岳の1,591mと2,811m）
- **確認なしでマージしてよい範囲を決めた（PR #29）** → `docs/workflow.md`
...
>   直すのは**データ側**（分け方そのもの）
> - 受け入れ条件: `node tests/run-all.js` 全18件PASS
