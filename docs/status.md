...
| 段階 | 中身 |
|---|---|
| **A** | ✅ **済（v4.86.0）。** 気温パネルの下端 11px を体感温度の色で塗り、判定が B/C の時間を枠で囲む。⚠ **色の目盛りは固定**（日ごとに伸縮させない）。⚠ `FEEL_BAND_H` は uPlot の `padding` とガターの `skyBottom` の**2か所と対**。⚠ 判定は**寒い側だけ**（暑い側は色で伝わるが枠は付かない） |
...
> docs/status.md を読んだうえで、**帯（下の赤黄緑）を消すか**を実機で見るところから進めて。
