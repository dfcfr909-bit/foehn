...
- ⚠ **タグが1本も無い**（リポジトリを作り直したときに引き継がれなかった）。
  「あの時動いていた版」を1ファイルで取り出す道が**いま塞がっている**。
  まず現行に1本打つこと（配信中の main = `488b684`）。**手元から実行する**:
  ```
  git tag -a v4.77.0 488b684 -m "v4.77.0" && git push origin v4.77.0
  ```
  ⚠ **Claude のセッションからは打てない。** タグの push は 403 で拒否される
  （ブランチは push できるのでタグ ref だけの制限。2026-08-14 に踏んだ）。
  旧 `SotoKi` のタグの移植は**しない**と決めた（SHAが全部別物で対応付けが手作業になる）。
  過去は諦めて今から積む
