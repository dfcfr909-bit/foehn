...
「下の赤黄緑の帯を無くしたい」という提案の途中。降水は済み（PR #63）。残りはこの順。

| 段階 | 中身 |
|---|---|
| **A** | ⚠ **体感温度をグラフに描く。** いまは画面に無い。**これが済むまで帯は消せない**——冬の稜線で寒さだけが原因のCが画面から消えるため。案は「気温パネル下端の細い帯に体感温度のグラデーション」。⚠ 塗るのは気温ではなく**体感温度**（判定に効いているのはこちら）。⚠ 背景は昼夜シェーディングが使っているので**背景を奪わない** |
...
> docs/status.md を読んだうえで、**体感温度をグラフに描く**ところから進めて。
