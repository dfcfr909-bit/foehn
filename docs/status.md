...
- **気象庁の天気図を別パネルで出すようにした（PR #44）。**
  実況＋24/48時間予想、日本周辺／アジア広域。⚠ **前線が読める**のが最大の利点。
  - ⚠ **計算した気圧配置は作ったうえで外した** → `docs/adr/0010-pressure-layer-removed.md`。
    429・重さ・角張り・拡大で消えるを直しきってなお「実用に耐えない」だった。
    **格子点APIで総観規模を毎回ブラウザから描く形が、このアプリと噛み合っていない**
...
⚠ **まず実機で至仏山を見てもらうこと（#16）。** ADR-0011 で判定に使う風が変わった。
ポップアップの風速の右が `@2228m` になっていれば新しい版が届いている
（`@地上10m（山頂2228m）` ならキャッシュが残っている）。
⚠ **穏やかな日はどちらもAなので、判定の字ではなくこの小さい表示で見る。**

> docs/status.md を読んだうえで、**山域の分け方の見直し**を進めて。
