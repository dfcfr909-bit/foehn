...
- **座標検証の残り1峰（#13）。** 笙ヶ岳（1,635m・鳥海山）だけ決まっていない。
  **`elev` 1635 は正しい**（実機の地図に三角点 △1634.9 が見えている）。
  間違っているのは座標のほうで、現在の 39.0972,140.0089 は**山頂の北東の斜面**
  （DEM 1,510m）を指している。三角点は南西に1km以上離れている。
  - `snapPeaks` は半径6kmに `1,635m±15m` の局所最高点を見つけられない。
    一致窓を30mに広げると 39.0971,140.0042（1,664m・**Δ+29m**・移動0.40km）を掴むが、
    **これは採ってはいけない**。DEMは山頂を低く読むので、公称より29m高い点は
    三角点ではなく別の高まり。地図上の△1634.9には距離的にも届いていない
  - ⚠ **`elev` をDEMの実測値に書き換えて辻褄を合わせないこと**（2026-08-10 の項）
  - **やること: 地図で △1634.9 を長押しし、`#map-latlon` の座標を読む。**
    機械では決められないと分かっている（推測で直さない → 2026-08-10 の項）
