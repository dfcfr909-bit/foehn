...
気温パネルの0℃の高さに**濃い青の点線**（`drawFreezingLine`・`FREEZING_LINE_COLOR`）。
氷点下に入る時間帯を一目で読むため（利用者の要望）。
- **`drawAxes` の hook** で引く＝背景の上・気温の線の下。線を隠さない
- 気温の目盛りの範囲に0℃が入らないとき（夏）は**引かない**。⚠ 範囲を0℃まで広げない（気温の線がつぶれる）
- 左の目盛り（`drawAxisGutter`）にも**「0°」を同じ濃い青で**出す。上下 12px 以内の目盛りは重なるので出さない（0°を優先）
- 検査は `tests/smoke_freeze.mjs`
...
- 枠は4辺すべて引く。降水のバーと違い、帯の下は**時刻ラベル帯**で、
  横線が「地面」に見える相手がいない
- ⚠⚠ **濃さは寒い側に寄せてある。** 判定（`abcScoreInv`）は**寒い側だけ**を見るので、
  暑い側に濃い色を使うと**その色に見合う危なさが判定側に無い**。実際、最初の目盛り
  （10℃中立・上下均等）では体感26〜31℃のふつうの夏日で帯が終日べったり橙になり、
  判定はAのままだった——`GRADE_COL.B` と紛らわしく「Aなのに警告色」に見えた
  - **8〜33℃ は淡い無彩色**。33℃ は**熱中症警戒アラート（暑さ指数 WBGT 33・環境省）**のあたり
  - 寒い側は **-3℃（判定Bの始まり）→ -15℃（判定Cの始まり）→ -25℃** と濃くしていく
  - ⚠ **寒い側の変化点は `THRESH.apparentA` / `apparentB` と同じ数値**。
    色が変わるところと判定が変わるところをずらさない（気圧で起きている食い違いを繰り返さない）。
    **THRESH を変えたら `FEEL_STOPS` も直す**——`smoke_feel.mjs` が突き合わせて落とす
  - ⚠ **暑い側の数値は暫定。** 帯が塗るのは WBGT ではなく `apparent_temperature` で、
    両者に公式の換算は無い。目安は湿度60%で 気温31℃ ≒ WBGT 28（厳重警戒）・
    気温35℃ ≒ WBGT 32（危険）。暑さ指数の式で当てると厳重警戒が体感35℃前後、
    危険が体感45℃前後。実測で詰めるなら Actions の probe から引く
  - ⚠ **暑い側の判定そのものが無い**（熱中症の警告を出していない）→ `status.md` の宿題
