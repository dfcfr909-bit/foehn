...
## 高度別の風の場（Wind Field Engine・v4.113.0）→ `docs/adr/0012-wind-field-reference-height.md`
...
- AUTO の区分（`trace.kind`）
  ...
  - `interp` で**間の層がデータなし**（GSM の期間の 800/900）なら `estimate: true`・`skipped` に飛ばした層（v4.115.0）。
    点線の矢印・「◯hPaが無い時刻のため間を飛ばして按分（推定値）」。⚠ 間の層が**モデル地中**で外れたのは飛ばしたに数えない
- 説明の窓に**山頂付近の目安**（v4.115.0）：升目の最高点（地形表の `max`）の高さでも同じ決め方で風を求めて1行添える
  （場には使わない）。上空で急に強まる日は、基準標高（稜線の帯）の風と山頂の風が倍以上違うことがある
  - `missing` … データなし（描かない）
...
- 速さは**ズームに依らない見た目の速さ**で、**風速に正比例**（1 m/s あたり `SPEED_PX` 0.16px／コマ。`windFlowPx`）。
  ⚠ v4.117.0 で √風速＋下限にしたら「ほぼ無風なのに勢いよく流れる」と言われ、v4.118.0 で戻した。無風の日は粒がほぼ止まるのが正しい。
  色は `windFlowColorIndex` で見た目の速さから風速に戻して決める。北が上なので v は符号を反転
...
### 地図のタイムスライダー（v4.119.0）

利用者の要望（「グラフと地図を行き来しないと時刻を変えられないのが面倒」）。`#map-time`（地点名の行のすぐ上）。

- **グラフと同じ選択時刻**（`state.sliderIndex`）を動かす（`setMapTime` → `setSelectedIndex(idx, true)`）。地図で選んだ時刻はグラフに戻っても同じ
- **出すのは風の層（流れ・矢印）が入っているときだけ**（`updateMapTime`。`refreshWeatherPoints` の最後で呼ぶ）。
  時刻で変わるのは風だけで、雨雲・雷・衛星・アメダスは実況（常に最新）。動かしても何も変わらない帯で地図を狭くしない（利用者の判断）
- 動かしている間（`input`）は日時の表示だけ変え、離したら（`change`）風をその時刻で描き直す（iPhone の負荷）。
  風は過去3日〜8日先を控えているので、範囲内なら取り直さない
- ◀▶は1時間ずつ（端で止まる）。「今」で現在時刻へ。過去の時刻は「過去（解析値）」と断る
- 帯に出すのは日時だけ（地点の気温・風・ABC は添えない。要否は実機で見てから。利用者の判断）
- 検査は `tests/smoke_maptime.mjs`
...
- **v4.111.0（利用者の指定）**：検索の履歴では数字を出さず **「（座標）」とだけ**。押すと**全画面の座標の画面**へ。
  ...
  「（Google mapに貼るならこれ！）」の注釈。
  ⚠ 略号の列は 72px（MGRS が収まる幅。60px だと呼び名に食い込んだ）
- **v4.112.0（利用者の指定）**：北緯・東経の行は**「読み」**（「和」は意味が分かりにくかった）。
  口頭で伝える（通報など）ための日本語の読み方で、座標形式そのものではない。
  **呼び名は全行に出し、意味のまとまりの途中で折り返さない**（呼び名は配列で1要素＝1行）——
  十進度／度・十進分／度分秒／読み／**ユニバーサル｜横メルカトル**／**軍事グリッド｜参照系**。
  呼び名の列は 70px（「ユニバーサル」「横メルカトル」が収まる幅）。
  ⚠ **360px 前後の画面**では、MGRS の値がコピーのボタンの下に潜り、「読み」の「秒」だけが次の行に落ちた。
  375px 未満では略号の列を 64px・字を 20px に、読みの値を 11px にし、MGRS は画面上だけ「54S WK｜00179 11120」の2行に分ける
  （コピーは1行のまま）。検査は 360／390px で見る
