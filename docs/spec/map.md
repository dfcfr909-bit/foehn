...
- **v4.110.0：UTM座標と MGRS を足し、見出しを利用者の指定した「UIでのおすすめ表記」にした**——
  十進度（DD）／度・十進分（DDM）／度分秒（DMS）／度分秒（北緯・東経）／UTM座標／MGRS。
  - UTM はクリューゲル級数（`toUTM`）、MGRS はそこから100km四方の記号を組み立てる（`fmtMGRS`）。
    ⚠ **答え合わせは pyproj（EPSG:326xx/327xx）と Python の mgrs ライブラリ**で作った値（14地点・1cm以内）。
    検査の値を直すときは同じ道具で作り直すこと（手で直さない）
  - ⚠ **1m 未満は切り捨て**（MGRS の決まり。UTM もそろえる）
  - ⚠ 帯の例外（ノルウェー 32V・スバールバル 31X/33X/35X/37X）と、経度ちょうどの境目（138° は 54 帯）
  - ⚠ 南緯80°〜北緯84°の外は UTM・MGRS の範囲外（極は UPS）。**黙って消さず「範囲外」と出す**
- **v4.111.0（利用者の指定）**：検索の履歴では数字を出さず **「（座標）」とだけ**。押すと**全画面の座標の画面**へ。
  行は **[略号を大きく・左端・上下中央][日本語の呼び名][値][コピー]**。DD には
  「（Google mapに貼るならこれ！）」の注釈。北緯・東経の行は略号が無いので「和」／度分秒（漢字）。
  ⚠ 略号の列は 72px（MGRS が収まる幅。60px だと呼び名に食い込んだ）
