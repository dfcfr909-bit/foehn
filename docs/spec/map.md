...
高気圧 **H(青)** ／低気圧 **L(赤)** と等圧線。Open-Meteo で `pressure_msl` を
まとめ取りする（`loadPressureGrid`）。**新しい情報源は増やしていない。**

- ⚠⚠ **格子は「画面の四隅」ではなく固定の升目に載せる**（`pressureBox`）。
  `PRESS_STEPS_DEG` の中から、点数が `PRESS_MAX_POINTS`(130) に収まる刻みを選び、
  表示範囲をその倍数へ切り上げ／切り下げる。
  **画面から格子を作ると、少し動かすたびに点の緯度経度が変わってキャッシュが
  一度も効かず、実機で `HTTP 429`（取りすぎ）を出した**（2026-08-20）。
  地図タイルが固定の升目に切られているのと同じ考え方
- ⚠ **キャッシュの鍵に画面の座標を入れない**（升目と時刻だけで決める）。
  控えは `PRESS_CACHE_MAX`(8) 枚のLRU。行き来しても取り直さない
- ⚠ **429 は「取りすぎ」。取り直しても解けない**ので `PRESS_COOLDOWN_MS`(90秒) 休み、
  **あと何秒待つかを言葉で出す**（`HTTP 429` とだけ出さない）
- 同じ升目を取りに行っている最中は相乗りする（連続した移動で二重に飛ばさない）
- ⚠ **行数(`n`)と列数(`nx`)は違う。** 升目は表示範囲に合わせて長方形になるので、
  片方だけで添字を計算すると**画面が横長のときだけ H の位置が飛ぶ**
...
- ⚠ **ズームで打ち切らない。** 以前は z8 を超えると何も描かなかったが、
  **山を見る縮尺で気圧配置が消えるのは使い勝手として逆**だった（実機で指摘）。
  代わりに `pressureBox` が、画面が狭くても `PRESS_MIN_SPAN_DEG`(24度) の広さで計算する。
  はみ出した線は地図が切るので無駄にならない
- ⚠ **升目1つぶんの余白を足して取る。** 縁の点は極値と判定しないので、
  画面いっぱいに取ると**画面端の高気圧・低気圧が永久に出ない**
- ⚠ **描くときは格子を `PRESS_REFINE`(4) 倍に細かくする**（`refineGrid`・Catmull-Rom）。
  等圧線は格子点の間を直線で結ぶので、点が少ないと**多角形に見える**（実機で
  「輪郭がやたらと角張っている」と指摘）。**通信は増えない**（間を埋めるだけ）。
  ⚠ **線形補間では滑らかにならない。** marching squares 自体が辺の上で線形補間しているので、
  同じ形の折れ線が細かくなるだけ。3次で埋める必要がある
- ⚠⚠ **補間は見た目を滑らかにするだけで、精度は上がらない。**
  だから **H/L の位置は元の格子から採る**（`pressureExtremes(g)`）。
  滑らかな線に釣られて「細かく分かっている」と思わせないための線引き
- 等圧線は marching squares。⚠ **鞍部（対角だけが高い升）は2通りに引ける。**
  4隅の平均で向きを決める。決めずに固定すると**等圧線が×印に交差した図**になる
- ⚠ **等圧線は高さごとに1レイヤーにまとめる。** 1本の高さでも数十の線分に割れるので、
  線分ごとに `L.polyline` を作ると画面に数百のSVG要素が並び、地図を動かすたびに
  作り直して**目に見えて重くなる**（実機で指摘を受けた。実測 108→9レイヤー）
