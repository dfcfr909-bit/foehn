...
描画（矢印・次は粒子）はこの場しか見ない（10m なのか 850 なのかを知らない）。

| モード | 決め方 |
|---|---|
| **AUTO（既定）** | 基準標高 z_ref（`data/terrain_ref.json`：升目の地理院標高の90%点を隣とならした＝稜線の高さ）の風を、**その時刻・その地点の `geopotential_height`** で上下の面から U/V 補間（`WindVertical.auto`） |
| 手動 10m / 925 / 900 / 850 / 800 / 700 | その層そのもの（`WindVertical.layer`）。検証・経験者向け |

- ⚠ **AUTO と手動は同じ取得・同じ控え・同じ場**。切り替えても取り直さない（全部の層を1回で取る）
- ⚠ **固定表で高さを決めない。** 850hPa≒1,460m のような表は使わない（ABC判定の `WIND_LEVELS` と混ぜない）
- ⚠ **役割分担**：実際の地形（z_ref）で「どの高さの風を見せるか」、**モデル地形（`elevation=nan`）で
  「その面がモデル上で有効か」**。モデル地形より下の面は外挿値なので使わない（null ではなく普通の値で返る）
- AUTO の区分（`trace.kind`）
  - `interp` … 上下の有効な面を高さで按分
  - `gap` … 地表付近（モデル地形〜最下の有効な面）。**10m風と最下の面を高さで線形に按分**（`WindVertical.nearSurface`）。
    物理的に正しい形ではないが、答え合わせで最も誤差が小さかった。**推定値として点線の矢印・「不確かさあり」**。
    境界層のモデル等に替えるときは `nearSurface` だけを差し替える
  - `surface` … z_ref がモデル地形以下（谷・平地）／地形表に無い升目（海）→ 地上10m
  - `top` … 700hPa より高い（富士山など）→ 700hPa を使い推定値と断る
  - `missing` … データなし（描かない）
- 手動：**無い時刻（GSM の期間の 900/800）を別の層で埋めない** →「800hPa：この時間帯はデータなし」。
  モデル地中は値を出さず「地中」
- 層は `WIND_FIELD_LEVELS` の表で持つ（`models` に MSM/GSM のどちらにあるか）。**層を足すときは1行足す**
- 格子は **MSM 上空の格子**（120E/22.4N 起点、0.125°×0.1°）に固定。ズームが粗いほど k 点おき
  （`WIND_BANDS`：z10+ は1、z9 は2、z8 は4、z7 は8）。z7 未満は「拡大すると風の流れを表示します」
- 時刻は**完全一致だけ**。取った幅（`WIND_SPANS`：near＝昨日〜3日、full＝3日前〜8日）の外へスライダーを
  動かしたら full を取り直す。⚠ 以前は2日分しか無く、外の時刻を**別の時刻の値で黙って埋めていた**
- 矢印を押すと根拠（`windTraceText`）：基準標高・モデル地形・採用した面と按分の割合・各層のその時刻の高さ・モデル
...
地図を少し動かすたび・時刻を変えるたびに取り直していた。

- 格子は**地図に固定**（`windFieldLattice`）。動かしても同じ点は同じ鍵（`i:j`。ズームをまたいでも共有）
- 点ごとに**時刻の幅ごと**控える（`windCols`、30分）。時刻を変えても取らない
- 取るのは**控えに無い点だけ**。動き終わってから 500ms 待って。1回の地点数は `WIND_FETCH_MAX_POINTS`(40) まで
  （⚠ 110地点×18変数では HTTP 200 のまま本文が `timeoutReached` で切れた）
- **429 を受けたら1分待つ**（`windBackoffUntil`）。その間は控えだけ描き、「断られた・何秒で取り直す」と出す
- ⚠ 値が返らなかった点も**空で控える**。控えないと描き直すたびに取り直しが続く
- 1地点の変数は 17（10m 2＋5層×3）で、Open-Meteo の数え方では**1地点あたり約1.7回**
...
- 風は**高度別の風の場**の格子点に矢印を置く（上の節）。矢印の向きは `風向+180°`。
  粒子（Windy のような流れ）は同じ場を使う予定（未実装）
...
- **座標を押すと表記の窓**（`#coord-sheet`、v4.109.0）。**DD**（小数5桁）・**DMS**（秒は小数1桁＋N/E）・
  **DDM**（分は小数3桁）・**度分秒**（北緯／東経）を並べ、行ごとと「まとめて」コピーできる（`coordFormats` / `copyText`）。
  ⚠ 座標を押しても**その地点へは移らない**（行のクリックに伝えない）。
  ⚠ **繰り上がり**：丸めは最小単位（0.1秒・0.001分）の整数で行い、60秒・60分を出さない。
  ⚠ 画面では緯度・経度を2行に分ける（数字の途中で折り返さない）。コピーは1行。
  クリップボードが使えない端末は旧方式（選択してコピー）に落とし、値そのものも選べるようにしてある。
  検査は `tests/smoke_coord.mjs`
