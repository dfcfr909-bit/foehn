...
友人・第三者にアプリを渡すときに、**URL 1本で説明が済む**ようにした公開ページ。
`https://dfcfr909-bit.github.io/foehn/about.html`。

中身は「先に読んでほしいこと（免責）→ A/B/C の決まり方 → 山頂高度の風 →
データの取得先 → 経緯 → 未完了の項目 → リンク」。

- **入口はアプリの版数表示。** ⚠ 凡例の横幅に余裕が無いので、文字を足さず
