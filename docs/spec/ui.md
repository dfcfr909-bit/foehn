...
- **入口は2つ: フッターの `ℹ️` と、版数表示。**
  - ⚠⚠ **`ℹ️` に文字を入れない。** 6つ目に「説明」と入れると **360px 幅で
    全ボタンが2行になり、フッターが 47px → 61px に伸びる**（実測）。
    `flex: 0 0 auto` で幅を固定する（均等割りだと他の5つが細って同じことが起きる）
  - ⚠ 原文だけでは分からないので、**実際に描いて高さを測る**検査にしてある ⚠ 凡例の横幅に余裕が無いので、文字を足さず
