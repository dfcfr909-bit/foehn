...
⚠ **気象APIをここへ足さないこと。** 古い数字が黙って出るようになる。
圏外で予報を出すのは**アプリ側の控え**の役目で、そちらは取得時刻と取得地点を
画面に出せる（→ `data.md`「圏外の控え」）。

HTML をネットワーク優先にしてあるので、「更新したのに古い画面のまま」は起きない。

⚠ **キャッシュに入れてよいのは `res.ok` の応答だけ。** 404や500を焼き付けると、
以後は圏外でもそのエラーが返り、**「圏外でも画面が立ち上がる」という目的が壊れる**。
HTML・静的ファイル・タイルの3つとも `res.ok` を見てから保存する
（HTMLだけ見落としていて、2026-08-13 に直した）。
